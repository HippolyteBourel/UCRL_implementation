...
class C_UCRL(C_UCRL_C):
	...
	def near(self, index, p_estimate, r_estimate, p_estimate_init, r_estimate_init, Nk_init, size):
		k = -1
		dist_min = float("inf")
		...
		for i in range(self.nC):
			if (i != index) and (self.Nk_c[i] != 0):
				...
				if (dist < 0) and (dist < dist_min) and self.isValid(i, index, p_estimate_init, r_estimate_init, Nk_init, size, p_estimate, r_estimate):
					dist_min = dist
					k = i
		return k
	...
	def merge(self, i, j, size, p_estimate, r_estimate):
		for s in range(self.nS):# Modification here
			p_estimate[i, s] = (self.Nk_c[i] * p_estimate[i, s] + self.Nk_c[j] * p_estimate[j, s]) / (self.Nk_c[i] + self.Nk_c[j])
		r_estimate[i] = (self.Nk_c[i] * r_estimate[i] + self.Nk_c[j] * r_estimate[j]) / (self.Nk_c[i] + self.Nk_c[j])
		size[i] += size[j]
	...
	def isValid(self, i, j, p_estimate_init, r_estimate_init, Nk_init, size, p_estimate, r_estimate):
		...
		#######################################################################################
		# Compute the future esitmate
		next_p_estimate = np.zeros(self.nS)
		for s in range(self.nS):
			next_p_estimate[s] = (self.Nk_c[i] * p_estimate[i, s] + self.Nk_c[j] * p_estimate[j, s]) / (self.Nk_c[i] + self.Nk_c[j])
		next_r_estimate = (self.Nk_c[i] * r_estimate[i] + self.Nk_c[j] * r_estimate[j]) / (self.Nk_c[i] + self.Nk_c[j])
		Nk = self.Nk_c[i] + self.Nk_c[j]
		# And now verify that each sample of the candidate future cluster are beta close from the future estimates
		for sample in (samples_i + samples_j):
			dist_p = (self.diffNorme1(next_p_estimate, p_estimate_init[sample])
					  - self.bW(Nk, delta) - self.bW(Nk_init[sample], delta))
			dist_r = (abs(next_r_estimate - r_estimate_init[sample])
					  - self.bH(Nk, delta) - self.bH(Nk_init[sample], delta))
			dist = max((dist_p, dist_r))
			if dist > 0:
				return False
		return True
	...
	def clustering(self, p_estimate, r_estimate):
		...
		#print("p_est = ", p_estimate, "\n r_est = ", r_estimate, "\n p_dist = ", self.p_distances)
		...
		while changed:
			...
			for i in ordering:
				if self.Nk_c[i] == 0:
					break
				k = self.near(i, p_estimate, r_estimate, p_estimate_init, r_estimate_init, Nk_init, size)
				if k != -1:
	...
	def new_episode(self):
		#print("Classes at time step ", self.t, " is : \n", self.C)
		...
		self.r_estimate_c = r_estimate_c


class C_UCRL_sqrtC(C_UCRL):
	def name(self):
		return "C_UCRL_sqrtC"
	...
	def isValid(self, i, j, p_estimate_init, r_estimate_init, Nk_init, size, p_estimate, r_estimate):
		[samples_i, samples_j] = self.get_pairs([i, j])
		# New sqrt criterion in the clustering
		if ((self.Nk_c[i] / size[i]) < np.sqrt(self.Nk_c[j] / size[j])) or ((self.Nk_c[j] / size[j]) < np.sqrt(self.Nk_c[i] / size[i])):
			return False
		delta = self.delta / self.nC # When this function is called we have self.nC = nS * nA
		for sample_i in samples_i:
			for sample_j in samples_j:
...
class C_UCRL_div4C(C_UCRL):
	...
	def isValid(self, i, j, p_estimate_init, r_estimate_init, Nk_init, size, p_estimate, r_estimate):
		[samples_i, samples_j] = self.get_pairs([i, j])
		# New div4 criterion in the clustering
		if ((self.Nk_c[i] / size[i]) < (self.Nk_c[j] / size[j]) / 4) or ((self.Nk_c[j] / size[j]) < (self.Nk_c[i] / size[i]) / 4):
			return False
		delta = self.delta / self.nC # When this function is called we have self.nC = nS * nA
		for sample_i in samples_i:
			for sample_j in samples_j:
...
class C_UCRL_div2C(C_UCRL):
	...
	def isValid(self, i, j, p_estimate_init, r_estimate_init, Nk_init, size, p_estimate, r_estimate):
		[samples_i, samples_j] = self.get_pairs([i, j])
		# New div4 criterion in the clustering
		if ((self.Nk_c[i] / size[i]) < (self.Nk_c[j] / size[j]) / 2) or ((self.Nk_c[j] / size[j]) < (self.Nk_c[i] / size[i]) / 2):
			return False
		delta = self.delta / self.nC # When this function is called we have self.nC = nS * nA
		for sample_i in samples_i:
			for sample_j in samples_j:
